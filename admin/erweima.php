...
<form onsubmit="return searchOrder()" method="GET" class="form-inline">
  <div class="form-group">
    <label>搜索</label>
	<select name="column" class="form-control" default="<?php echo $_GET['column']?$_GET['column']:'jiage'?>">
	<option value="jiage">金额</option>
	</select>
  </div>
  <div class="form-group">
    <input type="text" class="form-control" name="value" placeholder="搜索内容" value="<?php echo @$_GET['value']?>"><br>
	范围搜索如：5>6 为5-6的价格,或者填写 5 为查一个价格。
  </div>

 
  <button type="submit" class="btn btn-primary">搜索</button>
  <a href="javascript:searchClear()" class="btn btn-default" title="刷新二维码列表"><i class="fa fa-refresh"></i></a>
 
<?php if($DB->getRow("SHOW TABLES LIKE 'pay_order_old'")){echo '<a href="./order_old.php" class="btn btn-default btn-xs">历史二维码查询</a>';}?>
</form>
...
function listTable(query){
	...
	if(query == 'start' || query == undefined){
		query = '';
		history.replaceState({}, null, './erweima.php');
	}else if(query != undefined){
		history.replaceState({}, null, './erweima.php?'+query);
	}
	layer.closeAll();
	var ii = layer.load(2, {shade:[0.1,'#fff']});
	$.ajax({
		type : 'GET',
		url : 'erweima-table.php?dstatus='+dstatus+'&'+query,
		dataType : 'html',
		cache : false,
	...
	$.ajax({
		...
		success : function(data) {
			...
			if(data.code == 0){
				...
				item += '<tr><td colspan="6" style="text-align:center" class="orderTitle"><b>二维码信息</b></td></tr>';
				item += '<tr class="orderTitle"><td class="info" class="orderTitle">系统二维码号</td><td colspan="5" class="orderContent">'+data.trade_no+'</td></tr>';
				item += '<tr><td class="info" class="orderTitle">商户二维码号</td><td colspan="5" class="orderContent">'+data.out_trade_no+'</td></tr>';
				item += '<tr><td class="info" class="orderTitle">接口二维码号</td><td colspan="5" class="orderContent">'+data.api_trade_no+'</td></tr>';
				...
				item += '</tr><tr><td class="info" class="orderTitle">二维码金额</td><td colspan="5" class="orderContent">'+data.money+'</td></tr>';
				...
				item += '<tr><td class="info" class="orderTitle">二维码状态</td><td colspan="5" class="orderContent">'+status[data.status]+'</td></tr>';
				if(data.status>0){
				...
				item += '<tr><td colspan="6" style="text-align:center" class="orderTitle"><b>二维码操作</b></td></tr>';
...
function refund(trade_no) {
	...
	$.ajax({
		...
		success : function(data) {
			layer.close(ii);
			if(data.code == 0){
	var confirmobj = layer.confirm('此操作将从该商户扣除二维码分成金额，你需要手动退款给购买者，是否确定？', {
		btn: ['确定','取消']
	}, function(){
		var ii = layer.load(2, {shade:[0.1,'#fff']});
		$.ajax({
...
function setStatus(trade_no, status) {
	if(status==5 ){
		var confirmobj = layer.confirm('你确实要删除此二维码吗？', {
			btn: ['确定','取消']
		}, function(){
...
function setStatusDo(trade_no, status) {
	...
	$.ajax({
		type : 'get',
		url : 'ajax.php',
		data : 'act=delerweima&trade_no=' + trade_no + '&status=' + status,
		dataType : 'json',
