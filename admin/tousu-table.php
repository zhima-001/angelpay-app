...
if(isset($_GET['order']) && !empty($_GET['order'])) {
	...
	$con='系统订单号 '.$_GET['order'].' 共有 <b>'.$numrows.'</b> 条投诉订单记录';
	...
	$numrows=$DB->getColumn("SELECT count(*) from pre_tousu WHERE{$sql}");
	$con='包含 '.$_GET['out_trade_no'].' 的共有 <b>'.$numrows.'</b> 条投诉订单记录';
	$link='&my=order&order='.$_GET['order'].'&out_trade_no='.$_GET['out_trade_no'];
}else{
	$numrows=$DB->getColumn("SELECT count(*) from pre_tousu WHERE 1");
	$sql=" 1";
	$con='共有 <b>'.$numrows.'</b> 条投诉订单记录';
}
...
<?php echo $con?>
        <table class="table table-striped table-bordered table-vcenter">
            <!--<th>商户号</th>-->
          <thead><tr><th>ID</th><th>商户ID</th><th>上游通道</th><th>支付方式</th><th>操作人</th><th>投诉单号</th><th>系统订单号</th><th>商户订单号</th><th>凭证上的金额</th><th>投诉凭证</th><th>添加时间</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
echo '<tr><td>'.$res['id'].'</td><td>'.$res['uid'].'</td><td>'.$res['channel'].'</td><td>'.display_type($res['paytype']).'</td><td>'.$res['admin'].'</td><td><b>'.$res['tousuorder'].'</b></td><td><b>'.$new_msg.'</b></td><td><b>'.$res['out_trade_no'].'</b></td><td><b>'.$res['money'].'</b></td><td><img style="width:60px" src="../'.$res['tousuimage'].'"></td><td>'.$res['createtime'].'</td><td>'.display_status($res['status'],$res['id']).'</td><td>'.$ss.'&nbsp;<a class="btn btn-xs btn-info" onclick="editframe('.$res['id'].','.$res['status'].')">编辑</a>&nbsp;<a class="btn btn-xs btn-danger" onclick="delItem('.$res['id'].','.$res['status'].')">删除</a>&nbsp;</td></tr>';
} 
?>
          </tbody>
        </table>
