...
if(isset($_POST['user']) && isset($_POST['pass'])){
	...
	$change_pass = md5($pass);
// 	var_dump($change_pass);
// 	exit();
	$info = $DB->getRow("SELECT password FROM pre_admin WHERE name='$user' LIMIT 1");

	if($info['password'] != $change_pass){

	    	exit("<script language='javascript'>alert('用户名或密码不正确！!');history.go(-1);</script>");
	}else{
	   $DB->exec("insert into `pre_log` (`uid`,`type`,`date`,`ip`,`city`) values (0,'登录后台','".$date."','".$clientip."','".$city."')");
		$session=md5($user.$change_pass.$password_hash);
		$expiretime=time()+604800;
		$token2=authcode("{$user}\t{$session}\t{$expiretime}", 'ENCODE', SYS_KEY);
		setcookie("admin_token", $token2, time() + 604800);
		setcookie("admin_user", $user, time() + 604800);
		$parameter = array(
  ...
  <div class="container" style="padding-top:70px;">
    <div class="col-xs-12 col-sm-10 col-md-8 col-lg-6 center-block" style="float: none;">
      <div class="panel panel-primary">
        <div class="panel-heading"><h3 class="panel-title">001用户登陆</h3></div>
