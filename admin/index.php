...
<div class="row">
      ...
      <div class="panel panel-info">
        <div class="panel-heading"><h3 class="panel-title" id="title">后台管理首页</h3></div>
          <ul class="list-group">
			...
			<li class="list-group-item"><span class="glyphicon glyphicon-tint"></span> <b>总计余额：</b><span id="usermoney"></span>元（实时更新）</li>
			<li class="list-group-item"><span class="glyphicon glyphicon-tint"></span> <b>结算总额：</b><span id="settlemoney"></span>元（实时更新）</li>
	...
	<div class="col-xs-12 col-lg-4">
	  ...
	  <div class="panel panel-success">
	    <div class="panel-heading"><h3 class="panel-title">支付方式收入统计（实时更新）</h3></div>
        ...
        <div>
	         <select onchange="changgezhifu(this)" id="zuixinde"><option selected="selected" >完成时间</option><option>创建时间</option></select>
	         <input type="button" value="新版" onclick="dianjixinban()">
	     </div>
	  <div class="panel panel-warning" style="overflow-y: scroll;overflow-x:scroll;">
	    <div class="panel-heading" style="width:1000%"><h3 class="panel-title">支付通道收入统计（实时更新）</h3></div>
...
<script>
	function dianjixinban(){
	     var ts = $("#zuixinde").val();
	     $.ajax({
        		type : "GET",
        		url : "ajax.php?act=getcount3",
        		dataType : 'json',
        		data:{"ts":ts},
        		async: true,
        		success : function(data) {
        			...
        			$("#paytype_head").empty();
        			$("#paytype_head").append('<th>日期</th>');
        			$("#paytype_list").empty();
        			
        			$("#channel_head").empty();
        		    $("#channel_head").append('<th>日期</th>');
                    $("#channel_list").empty();
	...
	function changgezhifu(that){
	  
	    if($(that).val()=="创建时间"){
	        $.ajax({
        		type : "GET",
        		url : "ajax.php?act=getcount2",
        		dataType : 'json',
        		async: true,
        		success : function(data) {
        			$('#title').html('后台管理首页');
