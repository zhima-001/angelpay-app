...
function delBatch(batch){
	var confirmobj = layer.confirm('是否将该批次所有结算记录删除？', {
	  btn: ['确定','取消']
	}, function(){
	var ii = layer.load(2, {shade:[0.1,'#fff']});
	$.ajax({
		type : 'POST',
		url : 'ajax.php?act=del_batch',
		data : {batch: batch},
		dataType : 'json',
		success : function(data) {
			layer.close(ii);
