...
<div class="form-group">
<label>客服qq:</label><br>
<input type="text" class="form-control" name="kfqq" value="" >
</div>
<div class="form-group">
<label> logo:</label><br>
<input type="text"   style="width:280px;float:left;"  id="logo"  class="form-control" name="logo" value=""><input type="file" name="photo" id="photo" value="" placeholder="" style="float:left">
<input type="button" onclick="postData();" value="上传图片" name="" style="width:100px;height:30px;">
<img src="'.$row['logo'].'" id="logo_img" width="80" height="30" style="">
</div>

<div class="form-group">
<label>是否开启漏单通知:</label><br><select class="form-control" name="loudan"><option value="0">0_漏单通知</option><option value="1">1_漏单不通知</option></select>
</div>

<div class="form-group">
<label>json下单地址:</label><br>
<input type="text" class="form-control" name="jsonpayurl" value="">
</div>

<div class="form-group">
<label>客服url:</label><br>
<input type="text" class="form-control" name="kfurl" value="">
</div>
<div class="form-group">
<label>结算账号:</label><br>
...
<div class="form-group">
<label>通道编码是否开启:</label><br><select class="form-control" name="userstype"><option value="0">0_关闭</option><option value="1">1_开启</option></select>
</div>
...
<div class="form-group">
<label>客服qq:</label><br>
<input type="text" class="form-control" name="kfqq" value="'.$row['kfqq'].'" >
</div>
<div class="form-group">
<label> logo:</label><br>
<input type="text" style="width:280px;float:left;" class="form-control" id="logo" name="logo" value="'.$row['logo'].'"><input type="file" name="photo" id="photo" value="" placeholder="" style="float:left">
<input type="button" onclick="postData();" value="上传图片" name="" style="width:100px;height:30px;">
<img src="'.$row['logo'].'" id="logo_img" width="80" height="30" style="">
</div>

<div class="form-group">
<label>是否开启漏单通知:</label><br><select class="form-control" name="loudan" default="'.$row['loudan'].'"><option value="0">0_漏单通知</option><option value="1">1_漏单不通知</option></select>
</div>

<div class="form-group">
<label>json下单地址:</label><br>
<input type="text" class="form-control" name="jsonpayurl" value="'.$row['jsonpayurl'].'">
</div>

<div class="form-group">
<label>客服url:</label><br>
<input type="text" class="form-control" name="kfurl" value="'.$row['kfurl'].'">
</div>
...
<div class="form-group">
<label>通道编码是否开启:</label><br><select class="form-control" name="userstype" default="'.$row['userstype'].'"><option value="1">1_开启</option><option value="0">0_关闭</option></select>
</div>
...
$key = random(32);
$sds=$DB->exec("INSERT INTO `pre_user` (`gid`, `key`, `account`, `username`, `money`, `url`, `addtime`, `settle_id`, `phone`, `email`, `qq`, `certno`, `certname`, `cert`, `mode`, `pay`, `settle`, `status`,`kfqq`,`logo`,`kfurl`,`userstype`,`jsonpayurl`,`loudan`) VALUES ('{$gid}', '{$key}', '{$account}', '{$username}', '{$money}', '{$url}', '{$date}', '{$settle_id}', '{$phone}', '{$email}', '{$qq}', '{$certno}', '{$certname}', '{$cert}', '{$mode}', '{$pay}', '{$settle}', '{$status}','{$kfqq}','{$logo}','{$kfurl}','{$userstype}','{$jsonpayurl}','{$loudan}')");
$uid=$DB->lastInsertId();
if($sds){
	if(!empty($_POST['pwd'])){
...
$sql="update `pre_user` set `gid` ='$gid',`account` ='{$account}',`username` ='{$username}',`settle_id` ='$settle_id',`money` ='{$money}',`url` ='{$url}',`email` ='$email',`qq` ='$qq',`phone` ='$phone',`certno` ='$certno',`certname` ='$certname',`cert` ='$cert',`mode` ='$mode',`pay` ='$pay',`settle` ='$settle',`status` ='$status',`kfqq` ='$kfqq',`logo`='$logo',`kfurl`='$kfurl',`userstype`='$userstype',`jsonpayurl`='$jsonpayurl',`loudan`='$loudan' where `uid`='$uid'";
if(!empty($_POST['pwd'])){
	$pwd = getMd5Pwd(trim($_POST['pwd']), $uid);
