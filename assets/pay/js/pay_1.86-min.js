...
function tipUser(b, a) {
	...
	var p = false;
	if ($(".scantida").length > 0) {
		var g = $(".scantida").attr("data-timeout"),
		f = 0,
		h = 0;
		var t = function() {
			f++;
			h = g - f;
			$(".scantida .lbminitue").text(Math.floor(h / 60) + "分");
			$(".scantida .lbseconds").text(h % 60 + "秒");
			if (f < g) {
				if (f > 60) {
	...
	if ($(".scanpay").length > 0) {
		...
		function() {
			if (document.visibilityState == "visible") {
				...
				g=300-parseInt(Date.parse( new Date() ).toString().substr(0,10))+parseInt($(".scantida").attr("addtime"));
				//g=1500;
				//console.log(g);
