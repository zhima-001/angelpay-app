...
{
    // 日志记录
    static public function log($msg, $obj = null)
    {
        $date = date('Y-m-d');
        $hour = date('G');
        $time = date('Y-m-d H:i:s');
        $dir = 'log/' . $date;
        if (!is_dir($dir)) { //需要先判断文件夹是否存
            @mkdir($dir, 0777, true);
        }
        $filename = $dir . '/' . $hour . '.log';
        $objJson = json_encode($obj);
        if (is_writable($filename)) {
            file_put_contents($filename, "$time -> $msg : $objJson \n", FILE_APPEND | LOCK_EX);
        } else {
            fopen($filename, "w");
            file_put_contents($filename, "$time -> $msg : $objJson \n", FILE_APPEND | LOCK_EX);
        }
    ...
    //RSA2 sign算法
    static public function rsa_sign_public_key($param, $public_key)
    {
        //整理参数
        ksort($param);
        reset($param);
        ...
        // 使用对方公钥,对签名进行加密
        $pubSec = RSA2::thirdPublicEncryptPublicKey($sign, $public_key);
        // 返回签名
        return $pubSec;
    }
    //RSA2 sign算法
    static public function rsa_sign($param)
    {
        //整理参数
        ksort($param);
        reset($param);
...
//RSA2 sign算法
    static public function rsa_sign2($param)
    {
        ...
        // 待签名数据
        $signStr = substr($signStr, 0, -1);
        //functions::json(-2, '回调失败, 对方返回结果: '. $signStr);

        // 使用私钥签名
        $sign = RSA2::privSign($signStr);
        //functions::json(-2, '回调失败, 对方返回结果: '. json_encode($sign));
        // 使用对方公钥,对签名进行加密
        $pubSec = RSA2::thirdPublicEncrypt($sign);
        // 返回签名
        return $pubSec;
    ...
    //RSA2 sign检查
    static public function rsa_verify_sign($param)
    {
        // var_dump($param);
        
        // 接收到对方的数据
        // 使用自己的私钥解密签名
        $thirdDec = RSA2::privDecrypt($param['sign']);
        // 组装参数原始信息
        ksort($param);
        reset($param);
        $signStr = '';
    ...
    static public function rsa_verify_sign_public_key($param,$public_key)
    {
        ...
        $signStr = substr($signStr, 0, -1);
        // 使用对方的公钥验证签名
        return RSA2::thirdPublicVerifySignByPubKey($signStr, $thirdDec, $public_key) == 1;
    }
