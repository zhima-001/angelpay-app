<?php
function curl_get($url, $proxy = null)
{
    $ch = curl_init($url);
    $httpheader[] = "Accept: */*";
...
function xieInfo($chuanshu)//写info信息
{

    global $DB;
    $return = $DB->exec("INSERT INTO `pre_rizhi` (`pid`,`ddh`,`henji`,`getinfo`,`ip`,`addtime`) VALUES (:pid,:ddh,:henji,:getinfo,:ip,:addtime)", [':pid' => $chuanshu['pid'], ':ddh' => $chuanshu['ddh'], ':henji' => $chuanshu['henji'], ':getinfo' => $chuanshu['getinfo'], ':ip' => $chuanshu['ip'], ':addtime' => time()]);
    $uid = $DB->lastInsertId();
    return $uid;
}


function orderip($ip, $price, $order_sn, $status)
{
    global $DB;
    $createtime = strtotime(date('Y-m-d', time()));
    $return = $DB->exec("INSERT INTO pre_order_ip (ip,price,order_sn,status,createtime) VALUES (:ip,:price,:order_sn,:status,:createtime)", [':ip' => $ip, ':price' => $price, ':order_sn' => $order_sn, ':status' => $status, ':createtime' => $createtime]);
}

function xieCuowu($msg, $id)
{
    global $DB;
    $henji = $DB->getColumn("SELECT `henji` FROM pre_rizhi WHERE id='{$id}' LIMIT 1");
    //var_dump($henji);exit;
    $DB->exec("UPDATE `pre_rizhi` SET `henji` =:msg WHERE `id`=:id", [':msg' => $henji . '' . date("Y-m-d H:i:s", time()) . '&nbsp;&nbsp;' . $msg . "<br>", ':id' => $id]);
}

function xieCuowu1($msg, $id)
{
    global $DB;
    $henji = $DB->getColumn("SELECT `henji` FROM pre_rizhi WHERE ddh='{$id}' LIMIT 1");
    //var_dump($henji);exit;
    $DB->exec("UPDATE `pre_rizhi` SET `henji` =:msg WHERE `ddh`=:id", [':msg' => $henji . '' . date("Y-m-d H:i:s", time()) . '&nbsp;&nbsp;' . $msg . "<br>", ':id' => $id]);
}
...
function http_post_data($action, $data_string)
{
    //这里，
    /*$sql= "insert into wolive_tests (content) values ('".json_encode($data)."')";
    $this->pdo->exec($sql);*/
    $token = "5132092329:AAGnEYXKpz5MbDwvmKoMwVfhS-_iEUSnMhQ";
    // $token = "5313902856:AAEIQRhZIH6DOc2itLEig_D9ojdtOCkiAgY"; //测试
...
function qingqiujson($url, $data_string)
{
...
function ddh($url)
{
    $arr_1 = explode("&trade_no=", $url);
    $arr_2 = explode("&", $arr_1[1]);
    return $arr_2['0'];
}

//旧的
function do_notify($url, $type = 1)
{
    global $DB;

    //$return = curl_get($url, '45.76.191.111:23128');
    if($type == 2){
        $return = curl_get($url);
    }else{
        $return = curl_get($url, '45.76.191.111:23128'); 
    ...
    $out_trade_no = ddh($url);
    $log['content'] = $return;//获取用户端回调内容
    $log['trade_no'] = $out_trade_no;//商家订单号

    if (strpos($return, 'success') !== false || strpos($return, 'SUCCESS') !== false || strpos($return, 'Success') !== false) {
        $log['title'] = '订单异步通知成功';//日志标题
        wirteLog($log);//写日志信息
        xieCuowu1('订单异步通知成功；' . $url . "<br>获取到的反馈信息是：" . $return, $out_trade_no);
        return true;
    } else {
        $log['title'] = '订单异步通知失败';//日志标题
        wirteLog($log);//写日志信息
        xieCuowu1('订单异步通知失败' . $url . "<br>获取到的反馈信息是：" . $return, $out_trade_no);
        if ($type == 1) {
            do_notify($url, $type = 2);
        } else {


            //下查商户信息:
            $uid = $DB->getColumn("SELECT uid,notify_url FROM pre_order WHERE trade_no='{$out_trade_no}' LIMIT 1");
            //查用户需要不要漏单通知的功能
            $user_loudan = $DB->getColumn("SELECT loudan FROM pre_user WHERE uid='{$uid}' LIMIT 1");
            if ($user_loudan != 1) {
                //根据uid查用户的群信息：
                $user_chatid = $DB->getColumn("SELECT chatid FROM pre_botsettle WHERE merchant='{$uid}' LIMIT 1");
                $parameter = array(
                    'chat_id' => $user_chatid,
                    'parse_mode' => 'HTML',
                    'text' => "商户订单号：" . $out_trade_no . "通知失败1！请站长检查！回调地址：" . $url
                );
                $sss = http_post_data('sendMessage', json_encode($parameter));
...
//新的回调
function do_notify_new($wai_url, $url, $type = 1)
{
    global $DB;
    //这里改成请求自己的服务器：

    $return = qingqiujson($wai_url, json_encode($url));
// var_dump($return);
    //$return = curl_get($url);
    $out_trade_no = ddh($url['notify']);
    $log['content'] = $return;//获取用户端回调内容
    $log['trade_no'] = $out_trade_no;//商家订单号
    ...
    if (strpos($return, 'success') !== false || strpos($return, 'SUCCESS') !== false || strpos($return, 'Success') !== false) {
        $log['title'] = '订单异步通知成功';//日志标题
        //wirteLog($log);//写日志信息
        xieCuowu1('订单异步通知成功；' . $url['notify'] . "<br>获取到的反馈信息是：" . $return, $out_trade_no);
        return true;
    } else {

        $log['title'] = '订单异步通知失败';//日志标题
        //wirteLog($log);//写日志信息
        xieCuowu1('订单异步通知失败' . $url['notify'] . "<br>获取到的反馈信息是：" . $return, $out_trade_no);
        if ($type == 1) {
            do_notify_new($wai_url, $url, $type = 2);
        } else {

            //下查商户信息:
            $uid = $DB->getColumn("SELECT uid FROM pre_order WHERE trade_no='{$out_trade_no}' LIMIT 1");
            //查用户需要不要漏单通知的功能
            $user_loudan = $DB->getColumn("SELECT loudan FROM pre_user WHERE uid='{$uid}' LIMIT 1");
            if ($user_loudan != 1) {
                //根据uid查用户的群信息：
                ...
                $parameter = array(
                    'chat_id' => $user_chatid,
                    'parse_mode' => 'HTML',
                    'text' => "商户订单号：" . $out_trade_no . "通知失败2！请站长检查！回调地址：" . $url['notify']
                );
...
{
    global $DB;
    $return = $DB->exec("INSERT INTO `pre_notify_log` (`title`,`content`,`addtime`,`trado_no`) VALUES (:title,:content,:addtime,:trade_no)", [':title' => $log['title'], ':content' => $log['content'], ':addtime' => time(), ':trade_no' => $log['trade_no']]);
    return $return;
}
...
// 			if(do_notify($url['notify'])){.
            ...
            /*if(!empty($conf['tongzhishanghu'])){
                $changes = do_notify_new($conf['tongzhishanghu'],$url);
           
            }else{*/

            $changes = do_notify($url['notify']);
            ...
            if (!empty($conf['tongzhishanghu2'])) {
                if (!$changes) {
                    $changes = do_notify_new($conf['tongzhishanghu2'], $url);
                }
            }
...
function changeUserMoney($uid, $money, $add = true, $type = null, $orderid = null)
{
    ...
    if ($uid == "1133" || $uid == "1062") {
        //更新orderip信息
        $res = $DB->exec("UPDATE pre_order_ip SET status='1' WHERE order_sn='{$orderid}'");
    }
