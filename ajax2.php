...
case 'notify':
	...
	$miyao = daddslashes(trim($_POST['miyao']));
	
	$row=$DB->getRow("select * from pre_hongbao where trade_no='$trade_no' AND miyao=$miyao limit 1");
	if(!$row)
	{
		exit('{"code":-1,"msg":"当前订单不存在！"}');
	}
/* 	$row=$DB->getRow("select * from pre_order where trade_no='$trade_no' limit 1");
	$DB->exec("update pre_order set status=1,endtime='".date("Y-m-d H:i:s",time())."' where trade_no='$trade_no'");
	...
	exit('{"code":0,"url":"'.($_POST['isreturn']==1?$url['return']:$url['notify']).'"}'); */
		$row=$DB->getRow("select * from pre_order where trade_no='$trade_no' limit 1");
	  $url="https://".$_SERVER['HTTP_HOST']."/pay/hongbaopay/notify/".$trade_no."/";
	$_POST['mishi'] = md5("aya".$trade_no.'7758521');
	
	//日志开始
	$Money = $row['money'];
	$shuju['name'] ='红包支付';
	$shuju['money'] = $Money;
	$shuju['trade_no'] = $trade_no;
	$shuju['time'] = time();
