...
function display_status($status,$notify,$huidiao_num){
	if($status==1)
...
?> 
	  <div class="table-responsive">
        <table class="table table-striped table-bordered table-vcenter">
          <thead><tr><th>系统订单号/商户订单号</th><th>商品名称</th><th>商品金额</th><th>支付方式</th><th>支付IP<br>访问次数</th><th>创建时间/完成时间</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
while($res = $rs->fetch())
{
    ...
    if($cishu>=2){
       $have = true; 
       echo '<tr><td>'.$res['trade_no'].'<br/>'.$res['out_trade_no'].'</td><td>'.$res['name'].'</td><td>￥ <b>'.$res['money'].'</b></td><td> <b><img src="/assets/icon/'.$paytypes[$res['type']].'.ico" width="16" onerror="this.style.display=\'none\'">'.$paytype[$res['type']].'</b></td><td><span style="color:red">'.$res['ip']."</br>".'('.$cishu.'次/今天'.')--'.$res['waiwangip'].'</span></td><td>'.$res['addtime'].'<br/>'.$res['endtime'].'</td><td>'.display_status($res['status'], $res['notify'], $res['huidiao_num']).'</td><td><a href="./record.php?type=3&kw='.$res['trade_no'].'" class="btn btn-info btn-xs">明细</a>&nbsp;<a href="javascript:callnotify(\''.$res['trade_no'].'\')" class="btn btn-success btn-xs">补单</a></td></tr>';
    }else{
       $have = false; 
       echo '<tr><td>'.$res['trade_no'].'<br/>'.$res['out_trade_no'].'</td><td>'.$res['name'].'</td><td>￥ <b>'.$res['money'].'</b></td><td> <b><img src="/assets/icon/'.$paytypes[$res['type']].'.ico" width="16" onerror="this.style.display=\'none\'">'.$paytype[$res['type']].'</b></td><td><span style="color:green">'.$res['ip']."</br>".'('.$cishu.'次/今天'.')--'.$res['waiwangip'].'</span></td><td>'.$res['addtime'].'<br/>'.$res['endtime'].'</td><td>'.display_status($res['status'], $res['notify'], $res['huidiao_num']).'</td><td><a href="./record.php?type=3&kw='.$res['trade_no'].'" class="btn btn-info btn-xs">明细</a>&nbsp;<a href="javascript:callnotify(\''.$res['trade_no'].'\')" class="btn btn-success btn-xs">补单</a></td></tr>';
    }
