...
callback = function (data) {
    if (!data) return;
   // var timeover = parseInt(data.closetime - data.servertime - 3);
    var timeover = parseInt(data.closetime -Math.round(new Date() / 1000)-3);
...
qrcode_timeout = function () {
    $("#show_qrcode").attr("src", 'includes/weipay/img/overtime.png');
    $("#show_qrcode").attr("alt", '二维码失效');
    $("#msg h1").html("支付超时 请重新提交订单");
}
...
show_desc = function (data) { //商品描述
    var html = '';
    html += getDescMode('商品', data.subject || data.orderid || data.ordersn);
    html += data.ordersn ? getDescMode('单号', data.ordersn) : '';
    html += getDescMode('金额', "￥" + data.money);
    html += getDescMode('云端单号', data.orderid);
    html += getDescMode('创建时间', myDate("y-m-d h:m:s", data.servertime));
    html += getDescMode('过期时间', myDate("y-m-d h:m:s", data.closetime));
    $("#desc").html(html);
}
