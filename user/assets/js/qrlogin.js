...
function ptuiCB(code,uin,sid,skey,pskey,superkey,nick){
	...
	var msg ='请扫描二维码';
	if(url=='jieb'){
      var address = "/user/jsajax/ajax2.php?act=qqjieb";
	}else if(url=='bind'){
	  var address = "/user/jsajax/ajax2.php?act=qqbind";
	 }else{
	  var address = "/user/jsajax/ajax.php?act=qqlogin";
	 }
	switch(code){
		case '0':
		    $.ajax({
            type : "POST",
            url : address,
            data : {qq:uin,skey:skey,nick:nick},
            dataType : 'json',
