...
class five
{
    ...
    public function __construct()
    {
        ...
        $this->token = "8493830587:AAGdzGmAGlXl9FXffFRpu4vjO1x3QRl9OqI"; //ma 机器人：@kefuguanlibot
            ...
            if ($kefu_info) {
               ...
               // 检查是否是取消置顶事件
                ...
                if (isset($message_kefu['text'])) {
                    ...
                    // 检查是否引用了消息，如果有则添加引用链接
                    if (isset($message_kefu['reply_to_message']) && !empty($quoted_message_link)) {
                    ...
                    if (!empty($message_kefu['caption'])) {
                        ...
                        // 转换@提及
                        $description = $processAtMentions($description, $message_kefu);
                    }
                    if (isset($message_kefu['reply_to_message'])) {
                         $description = "> 引用的消息内容: \n[点击查看原消息]($quoted_message_link)\n\n" . $description;
                    }
    ...
    public function userlogin($username,$password,$chatid){
        ...
        if (isset($data['data']['authToken'])) {
            ...
            $this->xiaoxi("登录失败".$loginUrl,$chatid);
    ...
    { 
        ...
        if($gongsi_kefu_info){
            ...
            $set_sql = "insert into pay_userchatrc (tg_from_id,username,password,gmail,chatid,createtime,userid,usertoken,updatetime,tg_user_name) values ('" . $tg_userid . "','" . $username . "','" . $password . "','" . $kefuemail . "','" . $chatid . "', '" . $createtime . "','" . $rc_userid . "','" . $usertoken . "','" .$updatetime . "','" .$tg_user_name . "')";
            $this->pdo->exec($set_sql);
       ...
       /*$authToken = "mGdVT6XWMggPECwwtzSrGelq-Tu3jUqCWHoA76vRwWK";
        ...
        if (isset($data['success']) && $data['success'] == true) {
            ...
            $set_sql = "insert into pay_userchatrc (tg_from_id,username,password,gmail,chatid,createtime,userid,usertoken,updatetime,tg_user_name) values ('" . $tg_userid . "','" . $name . "','" . $name . "','" . $email . "','" . $chatid . "', '" . $createtime . "','" . $rc_userid . "','" . $usertoken . "','" .$updatetime . "','" .$tg_user_name. "')";
            $this->pdo->exec($set_sql);
