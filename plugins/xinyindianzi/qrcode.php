...
function sendPostRequest($url, array $postData, $timeout = 30)
{
    // 将数组转换成 x-www-form-urlencoded 格式
    ...
    // 可选：如果是 HTTPS 并且有自签名证书，可能需要关闭证书验证
...
$native = array(
        ...
        "fxbackurl" => $huidiaourl."pay/xinyindianzi/return/".TRADE_NO.'/', 
         "fxfee" => $order['money'],
         "fxdesc"=>"vip",
         "fxip"=>getClientIP(0,true),
         "fxnotifystyle"=>"2"
); 
...
$submitData = sendPostRequest($api,$native);
...
\lib\Zhifu::csasahangss(1,json_encode($submitData),"信音电子支付","下单");
...
if($submitData['status'] == "1"){
    ...
    \lib\Zhifu::csasahangss(2,"单号:".TRADE_NO.":".json_encode($submitData),"信音电子支付","返回异常");
