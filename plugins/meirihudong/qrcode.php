...
$native = array(
        ...
        "pay_notifyurl" => $huidiaourl."pay/meirihudong/notify/".TRADE_NO.'/',
        "pay_callbackurl"=>$huidiaourl."pay/meirihudong/return/".TRADE_NO.'/',
        "pay_amount" => $order['money'],
        "pay_productname"=>"VIP",
        "pay_ip"=>getClientIP(0,true),
         //"notifyurl" => $huidiaourl."huidiao.php/".TRADE_NO,
); 
ksort($native); //排序
$md5str = "";
$stringSignTemp = "pay_amount=".$native['pay_amount']."&pay_applydate=".$native['pay_applydate']."&pay_bankcode=".$native['pay_bankcode']."&pay_callbackurl=".$native['pay_callbackurl']."&pay_memberid=".$native['pay_memberid']."&pay_notifyurl=".$native['pay_notifyurl']."&pay_orderid=".$native['pay_orderid']."&key=$md5key";

$sign =strtoupper(md5($stringSignTemp));// MD5(stringSignTemp).toUpperCase()
...
$submitData = \lib\Zhifu::httpPost($api, $native); 
...
\lib\Zhifu::csasahangss(1,json_encode($submitData),"南方新材支付","下单");
