...
function aesDecrypt($data, $key, $iv) {
 ...
 /**
     * 验签方法
     */
function signVerification($sign, $key, $iv) {
        try {
            $decryptedSign = aesDecrypt($sign, $key, $iv);
            return $decryptedSign !== false && !empty($decryptedSign);
        } catch (Exception $e) {
            return false;
        }
    }
...
if(empty($appxianzhi_url)){
 ...
 // 1. 构建支付参数
            $payParams = [
                'appId' => 'APP_1752388995903',
                'dockingNum' => $channel['appurl'],
                'outTradeNo' => TRADE_NO,
                'money' => $order['money'],
                'notifyUrl' => $huidiaourl."pay/chengduluqiao/notify/".TRADE_NO.'/',
                'name' => 'VIP',
                'returnUrl' =>$huidiaourl."pay/chengduluqiao/return/".TRADE_NO.'/',
            ];

            // 2. 生成签名
            $json = json_encode($payParams, JSON_UNESCAPED_UNICODE);
            $sign = aesEncrypt($json, $md5key, "2e119e58a526bc64");
            $payParams['sign'] = $sign;

            // 3. 加密整个payInfo
            $payInfoJson = json_encode($payParams, JSON_UNESCAPED_UNICODE);
            $payInfo = aesEncrypt($payInfoJson, $md5key, "2e119e58a526bc64");
            ...
            // 6. 发送请求
            $result = \lib\Zhifu::http_post_data_two( $api,json_encode($requestParams));
            ...
            if ($response['code'] == '200') {
                ...
                $encryptedPayInfo = $response['payInfo'];
                $decryptedPayInfo = aesDecrypt($encryptedPayInfo, $md5key, "2e119e58a526bc64");
 var_dump($decryptedPayInfo);
                $payResult = json_decode($decryptedPayInfo, true);
                var_dump($payResult);
                
                // 8. 验签
                $isValid = signVerification($payResult['sign'], $md5key, "2e119e58a526bc64");
                if ($isValid) {
                    echo "支付下单成功，支付链接：" . $payResult['payUrl'];
                    return $payResult;
                } else {
                    echo "验签失败";
                    return null;
...
\lib\Zhifu::csasahangss(1,json_encode($submitData),"成都路桥支付","下单");
...
if($tg_beizhu == "Tg"){
 ...
 if($submitData['success']){        
        $submitData2 = json_decode($submitData['response'],true);
        ...
        $arr = array(
           ...
           'pay_url' => $submitData2['data']['payUrl'],
           'type' => "0",
           "money"=>$order['money'],
 ...
 if($submitData['success']){         
    $submitData2 = json_decode($submitData['response'],true);
    $payUrl = $submitData2['data']['payUrl'];
    $payurl =$payUrl;
