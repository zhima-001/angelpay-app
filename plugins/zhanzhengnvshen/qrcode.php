...
//请求地址
$fxgetway = "https://api.posmgood.com/api/commonpay/pay";
...
$params = array(
    ...
    "money" =>  $order['money'], //支付金额 单位元
    "timeSpan"=>getUnixTimestamp(),
    "accountName"=>$merId,
    "callBackUrl" => "http://huitiao2.tshuitiao123.xyz/pay/zhanzhengnvshen/notify/".TRADE_NO.'/', //异步回调 , 支付结果以异步为准
    "returnUrl"=> "http://huitiao2.tshuitiao123.xyz/pay/zhanzhengnvshen/return/".TRADE_NO.'/', //异步回调 , 支付结果以异步为准
    "ip"=>getClientIP(),
    "productName"=>"VIP",
    "productId"=>$channel['appurl'],
);
...
$submitData = \lib\Zhifu::http_post_data_two($fxgetway,json_encode($params));
$r = json_decode($submitData, true); //json转数组
...
if ($r["state"] == 0) {
    $agent = strtolower($_SERVER['HTTP_USER_AGENT']);
     if(strpos($agent, 'iphone') || strpos($agent, 'android')){
        ...
        echo "<script>";
        echo "window.location.href='" .  $r["data"] . "'";
        echo ";</script>";
        exit; 
     }else{
        $DB->exec("INSERT INTO `pre_orderzhong` ( `order_sn`, `urlstr`) VALUES (:order_sn, :urlstr)", [':order_sn'=>TRADE_NO,':urlstr'=>$r["data"]]);
     }
} else {
    //echo $r['error'].print_r($backr); //输出详细信息
    echo $r['message']; //输出错误信息
    exit();
}
