...
foreach($myconfig as $k=>$v){
    if($k>1){
        $s_arr=explode("|",$v);
        $short[sprintf("%.2f",$s_arr[0])]=$s_arr[1];
    }
}
...
if($rand["url"]==1){
    $payurl='alipays://platformapi/startapp?appId=20000123&actionType=scan&biz_data={"s": "money","u": "'.$channel["appmchid"].'","a": "'.$money.'","m":"gm"}';
}
if($rand["url"]==2){
...
<head>
    <meta charset="utf-8">
    <?php if($order["type"]==1){ ?> <title>支付宝付款</title><?php } ?>
    <?php if($order["type"]==-1){ ?> <title>银行卡付款</title><?php } ?>
...
<h1 class="scanheader text-center" style="display: none">
    ...
    <?php } ?>
    <?php if($order["type"]==-1){ ?><img src='/static/assets/images/banklogo.png' style='height:40px;width:auto'/><span >云闪付、银联App转账</span><?php } ?>
</h1>
...
<div class="scanheader text-center" style="padding: 10px 0 0 0;height: 38px; line-height: 38px;background: #dbe8f9;border-bottom: 0px;margin: 0px;padding: 5px 0 0 0;">
    <img src="/assets/pay/img/logo.jpg?1=11" style="height: 30px;float: left;    padding-left: 10px;">
...
<div class="scanbody " style="background: #FFF;border-bottom: 0px;margin-top: 0px; padding-top: 1px;margin-bottom: 0px;">
    ...
    <div class="price" style='position: relative;margin-top: -10px;'>
        <a style='display:none;position: absolute; left: 20px;  top: 25px;font-size: 14px;' target='_blank' href='{$goods.kf}'>
            ...
            联系客服</a>
        ￥<span style="color:red;font-size:50px;text-align:center"><?=$rand["money"]?></span>
        <?php if($rand["url"]==0){ ?>
        <small style="font-size:45%;display:block;color:red;font-weight:900">付错金额不能开通.</small>
        <?php } ?>
        <?php if($rand["url"]>0){ ?>
        <small style="font-size:45%;display:block;color:#000;font-size:12px;font-weight:900">如无法跳转<?php if($rand["url"]==0){ ?>支付宝 <?php } ?> <?php if($rand["url"]==-1){ ?>微信 <?php } ?>，请截屏保存二维码，到<?php if($rand["url"]==0){ ?>支付宝 <?php } ?> <?php if($rand["url"]==-1){ ?>微信 <?php } ?>扫一扫付款</small>
        <?php } ?>
    ...
    <div class="qrimgcon" >
        ...
        <div class="mobtipbtn hidden-sm hidden-md hidden-lg" style="padding-top:10px">
            ...
            <a id="topay" style="    background-color: #4295d5; border-color: #4295d5;"  href='<?=$payurl?>' class="btn btn-primary btn-block"> 打开支付宝付款</a>
        ...
        <?php } ?>
        <div class="scantida" addtime="<?=$rand['time']?>" data-timeout="<?=$djs?>" data-orderid="<?=$rand["orderno"]?>" data-return_url="/pay_back?orderid={$order.trade_no}">
    ...
    <div class="scantip" style="display: none">
        ...
        <div class="tiptext">
            打开支付宝 [扫一扫]，输入金额: <strong><?=$rand["money"]?></strong>
            <br>
...
<script type="text/template" id="payTipDescTemplate">
    <div class="text-center">
        <h4>您的付款金额是</h4>
        <p>
            <span class="badge" style="font-size:36px;background-color:red;color:white;vertical-align:bottom"><?=$rand["money"]?></span>
        </p>
        <p class="help-block">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="help-block"><strong style='color:red;font-weight:900;'>我们承诺：只要付过款，必定会开通。</strong></p>
                <p class="help-block">99％的情况下，3分钟内到账</p>
                <p class="help-block">付款时请一定填对金额，否则将导致积分不到账,不到帐可联系<strong style='color:red;font-weight:900;'>售后客服qq<?=$conf["kfqq"]?></strong></p>
                <p class="help-block">或者点击下方头像联系在线售后客服</p>
                <p style="text-align: left; padding-left: 19px;font-weight: 900;color:#608da0">
                    <a id="kfbtn" href="{$shop.kflj}" target="_blank"><img style="height: 25px;margin-right:5px;"  src="/assets/pay/img/kf.jpg"/> 在线客服(24h)</a>
                </p>
            </div>
...
<script src="/assets/pay/js/pay_1.86-min.js?t=3"></script>
