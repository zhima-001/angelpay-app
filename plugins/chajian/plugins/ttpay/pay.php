...
<head>
    <meta charset="utf-8">
    {if $order.pay_content_type != 8} <title>支付宝付款</title>{/if}
    {if $order.pay_content_type == 8} <title>银行卡付款</title>{/if}
    ...
    <link rel="stylesheet" href="/static/assets/css/pay_1.86-min.css?5=61">
    <style>
        #qrcode img{
...
<h1 class="scanheader text-center" style="display: none">
    {if $order.pay_content_type != 8}
    <span class="alilogo" title="支付宝付款"></span>
    {/if}
    {if $order.pay_content_type == 8}<img src='/static/assets/images/banklogo.png' style='height:40px;width:auto'/><span >云闪付、银联App转账</span>{/if}
</h1>
...
<div class="scanheader text-center" style="padding: 10px 0 0 0;height: 38px; line-height: 38px;background: #dbe8f9;border-bottom: 0px;margin: 0px;padding: 5px 0 0 0;">
    <img src="/static/img/logo.jpg?1=11" style="height: 30px;float: left;    padding-left: 10px;">
    ...
    <p style="margin-top:2px;float: right;margin-right: 10px;line-height: 28px;height: 30px; text-align: center;vertical-align: middle;border-radius: 50px;  ">
        <a id="kfbtn" href="javascript:$('#payTipModal').modal('show')"  ><img style="height: 25px;margin-right:5px;margin-top: -5px;"  src="/static/img/kf.jpg?5=1"/> 在线(24h)</a>
    </p>
...
<div class="scanheader text-center" style="padding: 10px 0 0 0;height: 34px; line-height: 20px; border-bottom: 0px;margin: 0px;padding: 5px 0 0 0;">
    <div class="help-block text-center cpcon" style=" color:#333;font-size:12px;">
        ...
        <button class="btn btn-danger1 btn-xs cpamountbtn"  data-clipboard-action="copy" data-clipboard-text="{$order.trade_no}" data-copydone="已复制到剪切板" style=" background-color: #00a0e7;color: #FFF;">
            复制
        </button>
...
<div class="scanbody " style="background: #FFF;border-bottom: 0px;margin-top: 0px; padding-top: 1px;margin-bottom: 0px;">
    ...
    <p style="display:none;text-align: left; padding-left: 15px;font-weight: 900;color:#333">
        <a href="#" target="_blank" style="color:#333;text-decoration-line: none;"><img style="height: 25px;margin-right:5px;"  src="/static/img/qq.jpg"/> 在线客服 {:sysconf('site_info_qq')}</a>
    </p>
    ...
    padding-right: 15px;display: none'>
        ...
        <p class="help-block"><strong style='color:red;font-weight:900;'>任何时候，支付时请不要在微信或支付宝里留言、不要投诉，也不要联系收款方，否则永久封号！！！</strong></p>
        <p class="help-block">微信、支付宝付款后积分自动到账，整个过程无需人工干预，99％的情况下 积分3分钟内到账</p>
        <p class="help-block">付款时请一定填对金额，否则将导致积分不到账,不到帐可联系<strong style='color:red;font-weight:900;'>售后客服qq{:sysconf('site_info_qq')}</strong>，如果她有事不在，给客服妹子一点时间，她一定会在<strong style='color:red;font-weight:900;'> 8 小时</strong>内看到留言并处理回复，给你一个满意处理结果</p>
    ...
    <div class="qrimgcon" >
        ...
        <div id="qrcode" class="qrimg"  width="210" height="210" style="position: relative;text-align: center;">
            <!-- <img class="qrimgicon alipng" src="/static/assets/images/alilogo_small.png" alt="ali logo img" width="32" height="32"> -->
            {if $order.codeType == 0}
            <p id='aliicon' style="display:block !important;background:url('/static/assets/images/alilogo_small.png');background-repeat:no-repeat;background-size:100% 100%;width:32px;height:32px;position: absolute;left: 50%; top: 50%; margin-left: -15px; margin-top: -15px;background-color: #FFF;" />
            {/if}

            {if $order.codeType == 2}
            <p id='aliicon' style="display:block !important;background:url('/static/assets/images/wxlogo_small.png?1=1');background-repeat:no-repeat;background-size:100% 100%;width:32px;height:32px;position: absolute;left: 50%; top: 50%; margin-left: -15px; margin-top: -15px;background-color: #FFF;" />

            {/if}

            <!-- <img  id='aliicon' src="/static/img/aliicon.png" style="background-color: #FFF;top: 48%;position: absolute;left: 48%;height: 30px;"  > -->
        </div>

        {if $order.codeType == 0 || $order.codeType == 2}
        <!--show in mobile-->
        ...
        <div class="mobtipbtn hidden-sm hidden-md hidden-lg" style="padding-top:10px">
            <!--<a href="alipay://{$order.pay_url}" class="btn btn-primary btn-block">2. 点击支付宝扫一扫从相册打开付款</a>-->
            {if $order.codeType == 0}
                <a href="javascript:;" onclick="if(openAliApp && confirm('请不要关闭此网页！！\n付款后一定要切换回浏览器此页面，才能完成付款！')){location.href='alipay://{$order.pay_url}';}" class="btn btn-primary btn-block">2. 点击
                    支付宝
                    扫一扫从相册打开付款</a>
            ...
            {if $order.codeType == 2}
                <a href="javascript:;" onclick="if(openAliApp && confirm('请不要关闭此网页！！\n付款后一定要切换回浏览器此页面，才能完成付款！')){location.href='{$order.pay_url}';}" class="btn btn-primary btn-block">2. 点击
                    微信
                    扫一扫从相册打开付款</a>
        ...
        {if $order.codeType == 1}
        <div class="mobtipbtn hidden-sm hidden-md hidden-lg" style="padding-top:10px">
            <!-- <a id="topay" style="    background-color: #4295d5; border-color: #4295d5;"  href="javascript:;" onclick="toAliPay()" class="btn btn-primary btn-block"> 打开支付宝付款</a> -->
            <a id="topay" style="    background-color: #4295d5; border-color: #4295d5;"  href='{$order.pay_url}' class="btn btn-primary btn-block"> 打开支付宝付款</a>

        </div>
        {/if}
        {/if}

        <a   style="display:none;background-color: #4295d5; border-color: #4295d5;"  href="javascript:;" onclick="toAliPay2()" class="btn btn-primary btn-block"> 测试支付宝付款</a>


        {if $order.pay_content_type == 8}
        <h3 class="text-center">请用云闪付、网银App转账</h3>
        <div class="text-center" style="margin-top:15px;font-size:22px"><small>卡号：</small><strong>6222 6202 8000 0896128</strong></div>
        <p class="help-block text-center cpcon" style="margin:0">
            <button class="btn btn-danger btn-xs cpamountbtn1" data-clipboard-action="copy" data-clipboard-text="6222620280000896128" data-copydone="已复制，在支付时粘贴">
                <span class="glyphicon glyphicon-duplicate"></span>
                点我复制卡号
            </button>
            <span class="cpdone hide"></span>
            <input value="6222620280000896128" class="cpfailipt hide" readonly="" id="ipt_cardnum">
            <span class="cpfail hide"><button class="btn btn-info btn-xs cpfailbtn" data-clipboard-target="#ipt_cardnum">长按左边复制</button></span>
        </p>
        <div class="text-center" style="margin-top:15px;font-size:22px"><small>姓名：</small><strong>朱圆圆</strong></div>
        <p class="help-block text-center cpcon" style="margin:0">
            <button class="btn btn-danger btn-xs cpamountbtn1" data-clipboard-action="copy" data-clipboard-text="朱圆圆" data-copydone="已复制，在支付时粘贴">
                <span class="glyphicon glyphicon-duplicate"></span>
                点我复制姓名
            </button>
            <span class="cpdone hide"></span>
            <input value="朱圆圆" class="cpfailipt hide" readonly="" id="ipt_ownername">
            <span class="cpfail hide"><button class="btn btn-info btn-xs cpfailbtn" data-clipboard-target="#ipt_ownername">长按左边复制</button></span>
        </p>
        <p class="text-center" style="margin:10px;font-size:22px">
            <small>银行：</small><strong>交通银行</strong>
        </p>
        {/if}
        <div class="scantida" data-timeout="{$timeout}" data-orderid="{$order.trade_no}" data-return_url="/pay_back?orderid={$order.trade_no}">
    ...
    <div class="scantip">
        <div class="tiptext">
            分别复制卡号、姓名
            <br>
            <strong>到云闪付、网银App里粘贴完成付款</strong>
        </div>
    </div>
    {/if}
    ...
    <div class="scantip" style="display: none">
        ...
        <div class="tiptext">
            打开支付宝 [扫一扫]，输入金额: <strong>{$order.total_price}</strong>
            <br>
...
<script type="text/template" id="payTipDescTemplate">
    <div class="text-center">
        <h4>您的付款金额是</h4>
        <p>
            <span class="badge" style="font-size:36px;background-color:red;color:white;vertical-align:bottom">{$order.total_price}</span>
        </p>
        <p class="help-block">
...
<div class="footer"  style="background-color: #f1f4fb;padding-bottom: 4px;margin-top: 0px;">
    本页由天使支付集团提供，已提供 <span style="color:#8aadbe">{:sysconf('site_info_paycount')} 次</span>成功支付
</div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p class="help-block">付款时请一定填对金额，否则将导致积分不到账,不到帐可联系<strong style='color:red;font-weight:900;'>售后客服qq{:sysconf('site_info_qq')}</strong></p>
                <p class="help-block">或者点击下方头像联系在线售后客服</p>
                <p style="text-align: left; padding-left: 19px;font-weight: 900;color:#608da0">
                    <a id="kfbtn" href="{$shop.kflj}" target="_blank"><img style="height: 25px;margin-right:5px;"  src="/static/img/kf.jpg"/> 在线客服(24h)</a>
                </p>
            </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="help-block"><strong style='color:red;font-weight:900;'>我们承诺：只要付过款，必定会开通。</strong></p>
                <p class="help-block">99％的情况下，3分钟内到账</p>
                <p class="help-block">付款时请一定填对金额，否则将导致积分不到账,不到帐可联系<strong style='color:red;font-weight:900;'>售后客服qq211249898</strong></p>
                <p class="help-block">或者点击下方头像联系在线售后客服</p>
                <p style="text-align: left; padding-left: 19px;font-weight: 900;color:#608da0">
                    <a id="kfbtn" href="https://tawk.to/chat/60b1dd686699c7280da98b83/1f6rcho9p" target="_blank"><img style="height: 25px;margin-right:5px;"  src="/assets/pay/img/kf.jpg"/> 在线客服(24h)</a>
                </p>
            </div>
...
<script src="/static/assets/js/pay_1.86-min.js?t=3"></script>
...
<script>
    ...
    function toAliPay2(){
        ...
        location.href = 'alipays://platformapi/startapp?appId=09999988&actionType=toAccount&goBack=NO&amount='+money/100+'&userId=2088042281421811&memo=6666';
    }
