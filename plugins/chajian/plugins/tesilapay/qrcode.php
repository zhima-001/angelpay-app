...
/*$data = [
            ...
            "pay_bankcode" => $leixing,
            "pay_notifyurl" => "https://".$_SERVER['HTTP_HOST']."/pay/falalipay/notify/".TRADE_NO.'/',
            "pay_callbackurl" => "https://".$_SERVER['HTTP_HOST']."/pay/falalipay/return/".TRADE_NO.'/',
...
$params = [
    ...
    "mch_id" => $merId,
    "trade_type" => "AliPaySPC",
    "money" => $order['money'],
    "out_order_no" => TRADE_NO,
    "notify_url" => "https://".$_SERVER['HTTP_HOST']."/pay/tesilapay/notify/".TRADE_NO.'/',
    "back_url" => "https://".$_SERVER['HTTP_HOST']."/pay/tesilapay/return/".TRADE_NO.'/',
...
// var_dump($params);
// exit();
$resp = Http::post($api, $params);
//$resp = build_request($api, $data);
//echo '返回值:'.$resp;
$datas = json_decode($resp,true);
//var_dump($arr);
...
if(strpos($datas['message'],'guid') === false){
    ...
    //   exit();
//      // preg_match_all ("/action=(.*)JSON\" method/U", $data, $pat_array);
//       preg_match_all ("/<form  action=\"(.*)\" method=\"POST\">/U", $data, $pat_array);
//      // print_r($pat_array[0]);
//       $new_url = $pat_array[1][0];
//       //var_dump($pat_array);
//       //exit();
//       echo $new_url;
//       exit();
//   $resp = Http::post($new_url);
//   echo $resp;
    echo "<script>";
