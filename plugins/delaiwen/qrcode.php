...
// $tjurl="http://www.qlzf.xyz/Pay_Index.html";
$api = $channel['apiurl'];
// $huidiaourl = $conf['huidiaourl'];
$huidiaourl = $channel['huidiaourl'];
$native = array(
    "pay_memberid" => $merId, //商户号
    "pay_orderid" => TRADE_NO, //商户订单号
    "pay_applydate"=>date("Y-m-d H:i:s",time()),
     "pay_bankcode" => $channel['appurl'], //支付类型 此处可选项为 微信公众号：wxgzh   微信H5网页：wxwap  微信扫码：wxsm   支付宝H5网页：zfbwap  支付宝扫码：zfbsm 等参考API
  
    "pay_amount" => $order['money'], //支付金额 单位元
    "pay_notifyurl" => $huidiaourl."pay/delaiwen/notify/".TRADE_NO.'/', //异步回调 , 支付结果以异步为准
    "pay_callbackurl" => $huidiaourl."pay/delaiwen/return/".TRADE_NO.'/', //同步回调 不作为最终支付结果为准，请以异步回调为准
...
if($submitData['code'] == "1"){
    ...
    echo "<script>";
    echo "window.location.href='" . $submitData['pay_url']. "'";
    echo ";</script>";
    exit;
}else{
    echo "支付失败，请返回原下单路径重新下单！";
    exit();
}
...
header("Content-type: text/html; charset=utf-8");
$api="http://www.qlzf.xyz/Pay_Index.html";
$data = array(
    "pay_memberid" => $merId, //商户号
    "pay_orderid" => TRADE_NO, //商户订单号
    "pay_applydate"=>date("Y-m-d H:i:s",time()),
    ...
    "pay_amount" => $order['money'], //支付金额 单位元
    "pay_notifyurl" => "http://huitiao.freewing123.xyz/pay/delaiwen/notify/".TRADE_NO.'/', //异步回调 , 支付结果以异步为准
    "pay_callbackurl" => "http://huitiao.freewing123.xyz/pay/delaiwen/return/".TRADE_NO.'/', //同步回调 不作为最终支付结果为准，请以异步回调为准

);

//生成签名 请求参数按照Ascii编码排序
ksort($data);        //将参数数组按照参数名ASCII码从小到大排序
...
$data["pay_md5sign"] = strtoupper(md5($stringSignTemp)) ;
$data["pay_productname"]  = "test"; //商品名
...
if($arr['data']['pay_url']!='')
{
    $payurl = $arr['data']['pay_url'];
}

if($arr['data']['qrcode']!='')
{
    $payurl = $arr['data']['qrcode'];
...
<div class="scanheader text-center" style="padding: 10px 0 0 0;height: 38px; line-height: 38px;background: #dbe8f9;border-bottom: 0px;margin: 0px;padding: 5px 0 0 0;">
    <img src="<?php if($huiyuan['logo']!=""){echo $huiyuan['logo'];}else{?>/assets/pay/img/logo.jpg?1=11<?php
    }?>" style="height: 30px;float: left;    padding-left: 10px;">
