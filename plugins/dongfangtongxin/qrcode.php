...
// RSA签名函数（SHA256WithRSA）
function getRsaSign($data, $privateKey) {
    try {
        // 排除sign和sign_type字段，并过滤空值
        $signData = array();
        foreach ($data as $key => $value) {
            // 排除sign、sign_type字段，以及空值和null值
            if ($key != 'sign' && $key != 'sign_type' && $value !== '' && $value !== null && $value !== false) {
                $signData[$key] = (string)$value; // 确保所有值都是字符串
            }
        }
        ...
        // 检查私钥是否为空
        if (empty($privateKey)) {
            throw new Exception("私钥为空，无法进行签名");
        }
        ...
        // 记录错误信息
        error_log("RSA签名错误: " . $e->getMessage());
        throw $e;
    }
}

$api = isset($channel['apiurl']) && !empty($channel['apiurl']) ? $channel['apiurl'] : 'https://shop.qqhkp.com/api/pay/create';
...
if(strpos($agent, 'iphone') !== false || strpos($agent, 'android') !== false) {
    ...
    $device = 'wechat';
} elseif(strpos($agent, 'alipay') !== false) {
    $device = 'alipay';
}
...
$native = array(
    ...
    "notify_url" => $huidiaourl."pay/dongfangtongxin/notify/".TRADE_NO.'/',
    "return_url" => $huidiaourl."pay/dongfangtongxin/return/".TRADE_NO.'/',
    "name" => "VIP",
    "money" => number_format($order['money'], 2, '.', ''),
    "clientip" => getClientIP(0, true),
...
try {
    // 检查私钥是否存在
    if (empty($privateKey)) {
        throw new Exception("私钥未配置，请在后台设置 private_key");
    }
    
    $sign = getRsaSign($native, $privateKey);
    $native['sign'] = $sign;
} catch (Exception $e) {
    \lib\Zhifu::csasahangss(2, "签名失败: " . $e->getMessage(), "新支付接口", "签名错误");
    echo "签名生成失败: " . $e->getMessage();
    echo "<br>请检查：";
    echo "<br>1. 后台是否配置了 private_key（商户私钥）";
    echo "<br>2. 私钥格式是否正确（应该是base64编码的字符串，不包含PEM头尾）";
    echo "<br>3. 私钥是否与商户ID 60002 匹配";
    exit();
}

// 发送请求（增加超时时间到60秒）
$submitData = \lib\Zhifu::httpPost($api, $native, 60); 
...
\lib\Zhifu::csasahangss(1,json_encode($submitData),"新支付接口","下单");
...
// 解析返回数据
$responseData = null;
if(is_string($submitData)) {
    $responseData = json_decode($submitData, true);
...
if($tg_beizhu == "Tg"){
    if(isset($responseData['code']) && $responseData['code'] == 0) {
        $responsePayType = $responseData['pay_type'];
        $payInfo = $responseData['pay_info'];
        ...
        if($responsePayType == 'qrcode') {
            // 二维码支付
            $payUrl = $payInfo;
        } elseif($responsePayType == 'jump') {
            // 跳转支付
            $payUrl = $payInfo;
        } elseif($responsePayType == 'html') {
            // HTML代码，需要解析
            $payUrl = $payInfo;
        } else {
            $payUrl = $payInfo;
...
if(isset($responseData['code']) && $responseData['code'] == 0) {
    ...
    $payUrl = '';
    if($responsePayType == 'qrcode') {
        // 二维码支付，直接使用pay_info作为二维码内容
        $payUrl = $payInfo;
    } elseif($responsePayType == 'jump') {
        // 跳转支付，直接使用pay_info作为跳转URL
        $payUrl = $payInfo;
    } elseif($responsePayType == 'html') {
        // HTML代码，需要解析或直接使用
        $payUrl = $payInfo;
    } elseif($responsePayType == 'urlscheme') {
        // URL Scheme，用于小程序跳转
        $payUrl = $payInfo;
    } else {
        // 其他类型，直接使用
    ...
    if (isset($responseData['msg'])) {
        ...
        if (is_string($responseData['response'])) {
            $responseJson = json_decode($responseData['response'], true);
    ...
    \lib\Zhifu::csasahangss(2,"单号:".TRADE_NO.":".json_encode($responseData),"新支付接口","返回异常");
    $DB->exec("UPDATE pre_order SET ischeng='0' WHERE trade_no='".TRADE_NO."'");
    
    // 输出详细错误信息用于调试
    if (isset($responseData['code'])) {
        echo "拉取支付失败 (错误码: " . $responseData['code'] . "): " . $errorMsg;
    } else {
        echo "拉取支付失败: " . $errorMsg;
        echo "<br>返回数据: " . json_encode($responseData, JSON_UNESCAPED_UNICODE);
    }
    exit();
}
...
<style data-savepage-href="https://cdn.staticfile.org/layer/3.1.1/theme/default/layer.css?v=3.1.1">
