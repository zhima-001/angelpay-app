...
$count_order= $DB->getRow("SELECT count(*) as counts FROM pay_order WHERE trade_no = '".TRADE_NO."'");
$count_order=$count_order['counts'];
if($count_order>1){
      exit("禁止重复提交，请返回重新下单");
}
...
// $api = "http://206.233.128.100:32686/gateway/p.do";
$api = $channel['apiurl'];
...
$native = array(
    ...
    "reqid" => TRADE_NO,
    "money" => $order['money'],
    "reqts" => date('YmdHis',time()),
    "type" => $channel['appurl'],
    'returl' => $huidiaourl."pay/xiangnaier/notify/".TRADE_NO.'/',
...
//sign		签名：(md5(merid+reqid+reqts+type+money+returl+key))

//echo($md5str . "key=" . $Md5key);

$sign_str = $merId.TRADE_NO.$native['reqts'].$native['type'].$order['money'].$native['returl'].$md5key;
$sign = md5($sign_str);
...
if($submitData['result'] == "0"){
    ...
    $shuju1 = $submitData;
	$DB->exec("INSERT INTO `pre_notify_rizhi` (`content`,`addtime`, `jine`,`zt`) VALUES (:content,:addtime, :jine,:zt)", [':content'=>$shuju1."===>".$start_time,':addtime'=>time(),':jine'=>"0",':zt'=>'error']);
    echo "拉取支付失败";
    
    //给机器人发消息：
    $chatid = "-1001781426777";
    $parameter = array(
        'chat_id' => $chatid ,
        'parse_mode' => 'HTML',
        'text'=>$submitData
