...
$rand= $DB->getRow("SELECT * FROM pay_rand WHERE type = 'ddpay' and orderno = '".TRADE_NO."' LIMIT 1");
$money=$rand['money'];
...
<div class="scanheader text-center" style="padding: 10px 0 0 0;height: 34px; line-height: 20px; border-bottom: 0px;margin: 0px;padding: 5px 0 0 0;">
    <div class="help-block text-center cpcon" style=" color:#333;font-size:12px;">
        ...
        <button class="btn btn-danger1 btn-xs cpamountbtn"  data-clipboard-action="copy" data-clipboard-text="<?=$rand["orderno"]?>" data-copydone="已复制到剪切板" style=" background-color: #00a0e7;color: #FFF;">
            复制
        </button>
...
<div class="scanbody " style="background: #FFF;border-bottom: 0px;margin-top: 0px; padding-top: 1px;margin-bottom: 0px;">
    ...
    padding-right: 15px;display: none'>
        ...
        <p class="help-block">付款时请一定填对金额，否则将导致积分不到账,不到帐可联系<strong style='color:red;font-weight:900;'>售后客服qq211249898</strong>，如果她有事不在，给客服妹子一点时间，她一定会在<strong style='color:red;font-weight:900;'> 8 小时</strong>内看到留言并处理回复，给你一个满意处理结果</p>
    ...
    <div class="qrimgcon" >
        ...
        <?php } ?>
        <div class="scantida" addtime="<?=strtotime($order['addtime'])?>" data-timeout="<?=$djs?>" data-orderid="<?=$rand["orderno"]?>" data-return_url="/pay_back?orderid={$order.trade_no}">
