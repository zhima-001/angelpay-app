...
for($x=0; $x<200; $x++){
    ...
    $row= $DB->getRow("SELECT * FROM pay_rand WHERE type = 'ddpay' and zfb ='".$channel["appurl"]."' and acc = '".$channel["appmchid"]."' and money='".$money."' LIMIT 1");
    if($row==""){
        $res=money($money,$myconfig,$arr,$short);
        if($res>0){
            $DB->exec("INSERT INTO `pay_rand` (`type`,`orderno`, `zfb`, `acc`, `money`, `status`, `url`, `time`) VALUES (:type,:orderno, :zfb, :acc, :money, :status, :url , :time)", [':type'=>'ddpay',':orderno'=>TRADE_NO,':zfb'=>$channel["appurl"],':acc'=>$channel["appmchid"],':money'=>$money,':status'=>0,':url'=>$res, ':time'=>time()]);
            http_post($money,TRADE_NO,$channel);
            break;
        }else{
            $arr["type"]=1;
            $arr["money"]=$money;
...
if($res==0){
    if(isset($other[0])){
        if($other[0]["type"]==2){
            ...
            http_post($other[0]["money"],TRADE_NO,$channel);
        }
        if($other[0]["type"]==1){
            $DB->exec("INSERT INTO `pay_rand` (`type`, `orderno`, `zfb`, `acc`, `money`, `status`, `url`, `time`) VALUES (:type,:orderno, :zfb, :acc, :money, :status, :url , :time)", [':type'=>'ddpay',':orderno'=>TRADE_NO,':zfb'=>$channel["appurl"],':acc'=>$channel["appmchid"],':money'=>$other[0]["money"],':status'=>0,':url'=>$res, ':time'=>time()]);
            http_post($other[0]["money"],TRADE_NO,$channel);
        }
        /*echo "金额：".$other2[0]."<br>";
        echo "任意码-";
...
function http_post($money,$orderno,$channel){
    $url="https://www.pays.top/app/pay.php";
    $data["gw_mchid"]=$channel["appid"];
    $data["gw_notify"]="https://".$_SERVER['HTTP_HOST']."/pay/ddpay/notify/".$orderno."/";
    $data["gw_order"]=$orderno;
    $data["gw_price"]=$money*100;
    ...
    $data["gw_sign"]=strtolower(md5($str."key=".$channel["appkey"]));
    $data["gw_extra"]="";
    $data["gw_return"]="";
...
echo "<script>window.location.href='/pay/ddpay/qrcode/{$trade_no}/?sitename={$sitename}';</script>";
/*for($x=0; $x<10; $x++){
