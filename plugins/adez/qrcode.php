...
//请求地址
//$fxgetway = "http://xd.88xdxd88.com/pay";
$api = $channel['apiurl'];//http://xd.88xdxd88.com/pay
...
$data = array(
    "fxid" => $merId, //商户号
    "fxddh" => TRADE_NO, //商户订单号
    "fxdesc" => "test", //商品名
    "fxfee" =>  $order['money'], //支付金额 单位元
    "fxattch" => 'mytest', //附加信息
    "fxnotifyurl" => $huidiaourl."pay/adez/notify/".TRADE_NO.'/', //异步回调 , 支付结果以异步为准
    "fxbackurl" => $huidiaourl."pay/adez/return/".TRADE_NO.'/', //同步回调 不作为最终支付结果为准，请以异步回调为准
    "fxpay" => $channel['appurl'], //支付类型 此处可选项为 微信公众号：wxgzh   微信H5网页：wxwap  微信扫码：wxsm   支付宝H5网页：zfbwap  支付宝扫码：zfbsm 等参考API
    "fxip" => getClientIP(0, true) //支付端ip地址
);
$data["fxsign"] = md5($data["fxid"] . $data["fxddh"] . $data["fxfee"] . $data["fxnotifyurl"] . $md5key); //加密
...
if ($r["status"] == 1) {
    $agent = strtolower($_SERVER['HTTP_USER_AGENT']);
     if(strpos($agent, 'iphone') || strpos($agent, 'android')){
        ...
        echo "<script>";
        echo "window.location.href='" .  $r["payurl"] . "'";
        echo ";</script>";
        exit; 
     }else{
