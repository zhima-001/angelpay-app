...
//查询订单是不是已经支付了：   
//if($order['status'] == "1"){
    // echo "<div style='margin-top:50%;text-align:center'><p style='font-size:45px'>当前订单已支付，正在为您跳转到支付页面，请稍候...</p></div>";
     ?>
...
//请求地址
// $api = "http://156.245.23.10:26688/api/pay"; //http://139.198.116.252:6789/api/pay
...
$native = array(
    ...
    "desc"=>"VIP会员",
    "ip"=>$order['ip'], //支付端ip地址
    "notifyUrl" => $huidiaourl."pay/dahaizhifu/notify/".TRADE_NO.'/', 
    "returnUrl" => $huidiaourl."pay/dahaizhifu/return/".TRADE_NO.'/', 
    "nonceStr"=>getstr()
);
ksort($native);
$md5str = "";
...
$neworder= $DB->getRow("SELECT * FROM pre_orderrecode WHERE order_sn = '".TRADE_NO."' LIMIT 1");
if(!$neworder){
    //$submitData = \lib\Zhifu::http_posts_data($native);
    $submitData = Http::post($api, $native);
}else{
    echo "订单重复，请重新返回原网站重新下单！";
    exit();
}
...
\lib\Zhifu::csasahangss(1,json_encode($arr),"大海支付","下单");
if($tg_beizhu == "Tg"){
