...
<body>
    ...
    <script>
        ...
        var ptimer = setInterval(function () {
            timer--;
            if(timer<=0){
                document.querySelector("#allp").innerHTML= "下单异常,请重新回到用户端下单支付";
            }else{
...
$DB->exec("UPDATE pre_order SET shijian=shijian+'{$end_time}' WHERE trade_no='$trade_no'");*/
//var_dump($channel);exit;
echo "<script>window.location.href='/pay/jiumuwang/qrcode/{$trade_no}/?sitename={$sitename}';</script>";
