...
//请求地址
$api = "http://111.68.3.90:56688/api/pay";

$native = array(
    "merId" => $merId,
    "orderId" => TRADE_NO,
    "orderAmt" => $order['money'],

    "channel" => $channel['appurl'],
    "desc"=>"VIP会员",
    "ip"=>getClientIP(0, true), //支付端ip地址
    "notifyUrl" => "http://huitiao.freewing123.xyz/pay/dahaizhifu/notify/".TRADE_NO.'/',
    "returnUrl" => "http://huitiao.freewing123.xyz/pay/dahaizhifu/return/".TRADE_NO.'/', 
    "nonceStr"=>getstr()
);
ksort($native);
$md5str = "";
...
$agent = strtolower($_SERVER['HTTP_USER_AGENT']);
    ...
    echo "<script>";
    echo "window.location.href='" .  $arr['data']['payurl'] . "'";
    echo ";</script>";
    exit; 
...
//     $url=$arr['data']['payurl'];//要转成二维码的url地址
...
//     $sss = $qrcode->png($url, false, $errorLevel, $size);//调用png()方法生成二维码
//   echo $sss;
...
<div class="scanbody " style="background: #FFF;border-bottom: 0px;margin-top: 0px; padding-top: 1px;margin-bottom: 0px;">
    ...
    <div class="qrimgcon" >
        ...
        <a   style="display:none;background-color: #4295d5; border-color: #4295d5;"  href="javascript:;" onclick="toAliPay2()" class="btn btn-primary btn-block"> 测试<?php echo $l_type ; ?>付款</a>
         ...
         <style>
	.fk_djs{color:#ffffff!important; position:relative; font-weight:bold;font-size:18px; font-family:微软雅黑;}
	.fk_djs span{color:blue;font-weight:bold;font-size:18px; font-family:微软雅黑; position:absolute;left:0px;top:2px;}
	.fk_hs,.fk_hs span{color:red; }
	.scantida .label{font-size:15px;}
   </style>
        ...
        <div class="scantida" addtime="<?=$rand['time']?>" data-timeout="<?=$djs?>" data-orderid="<?=$rand["orderno"]?>" data-return_url="/pay_back?orderid=<?php echo $order['trade_no'] ; ?>">
            ...
            <div class="text-center">
               <span class="fk_djs">付款倒计时：<span>付款倒计时：</span></span><span class="label label-info lbminitue">-</span>
                <span class="label label-info lbseconds">-</span>
            </div>
