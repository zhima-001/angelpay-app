...
//请求地址
$fxgetway = "http://8.130.101.182/api/pay";



$native = array(

    "merchant_no" => $merId, //商户号
    "out_order_no" => TRADE_NO, //商户订单号
  
    "amount" => (int)$order['money'], //支付金额 单位元
   
    "notify_url" =>  "http://huitiao2.tshuitiao123.xyz/pay/zhandanren/notify/".TRADE_NO.'/', //异步回调 , 支付结果以异步为准
    "pay_type" => $channel['appurl'], //支付类型 此处可选项以网站对接文档为准 微信公众号：wxgzh   微信H5网页：wxwap  微信扫码：wxsm   支付宝H5网页：zfbwap  支付宝扫码：zfbsm 等参考API

);
$sign=md5($merId.TRADE_NO.$native['amount'].$channel['appurl'].$native['notify_url'].$md5key);


$native["sign"] = $sign;

$return_data =  Http::post($fxgetway,$native);
...
if($submitData['code'] =="1"){
	...
	//$payurl = "http://huitiao.freewing123.xyz/gongxifacai.php?order_sn=".TRADE_NO;
        $payurl =   $submitData['data']['pay_url'] ;
    $agent = strtolower($_SERVER['HTTP_USER_AGENT']);
