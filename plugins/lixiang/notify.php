...
$returnArray = array( // 返回字段
 "price" =>  $_REQUEST["price"], // 流水号
     "sysOrderNo"=>$_REQUEST['sysOrderNo'],
    "merchantOrderNo" => $_REQUEST["merchantOrderNo"], // 商户ID

     "status"=>$_REQUEST['status'],
       "fee"=>$_REQUEST['fee'],
        "payTime"=>$_REQUEST['payTime'],
          "realMoney"=>$_REQUEST['realMoney'],
...
if($sign == $_REQUEST["sign"]){
    if($_REQUEST['status']=="PAY_STATUS_SUCCESS"){
        $flag = true;
    }else{
        exit('签名校验错误');
    }
...
\lib\Zhifu::csasahangss(1,json_encode($returnArray),"理想支付","回调");
if($flag){
    //处理逻辑
    $out_trade_no  = $_REQUEST['merchantOrderNo'];
