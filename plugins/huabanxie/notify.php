...
$flag = false;
$data = [];
$data['merchant_no'] = $_POST['merchant_no'];//商户号
$data['order_no']   = $_POST['order_no'];//第三方订单号
$data['platform_order_no']    = $_POST['platform_order_no'];//金额，元单位
$data['order_money']   = $_POST['order_money'];//平台订单号
...
if($keysign == $sign){
    ...
    exit;
        ...
        $data['name'] ='滑板鞋支付';
        $out_trade_no = daddslashes($data["order_no"]);
        //日志开始
	$shuju = $data;
	$Money = $data["pay_money"]/100;
...
if($flag){
	...
	$orderAmt = $Money;
	$trade_no = daddslashes($data["order_no"]);
