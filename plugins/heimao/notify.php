...
//完整示例：sign = MD5(key=M23378425142f5b0b-0499-4581-a8fd-239dd9bff248&order_amount=500.00&order_no=HTM202004130345300377663&pay_code=HTP210938)

$returnArray = array( // 返回字段
 "merchant_no" =>  $_REQUEST["merchant_no"], // 流水号
     "pay_code"=>$_REQUEST['pay_code'],
    "order_amount" => $_REQUEST["order_amount"], // 商户ID
     "order_actual_money" =>  $_REQUEST["order_actual_money"], // 交易时间
...
$md5str = "key=". $_REQUEST["merchant_no"].$md5key."&order_amount=". $_REQUEST["order_amount"]."&order_no=".$_REQUEST['order_no']."&pay_code=".$_REQUEST['pay_code'];
...
\lib\Zhifu::csasahangss(1,json_encode($returnArray),"黑猫支付","回调");
if($flag){
    //处理逻辑
    $out_trade_no  = $_REQUEST['order_no'];
    $Money =  $_REQUEST['order_amount'];
