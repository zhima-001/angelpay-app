...
$returnArray = array( // 返回字段
        "mchOrderNo" =>  $_REQUEST["mchOrderNo"], // 流水号
        "tradeNo"=>$_REQUEST['tradeNo'],
        "payAmount" => $_REQUEST["payAmount"], // 商户ID
        "orderStatus" =>  $_REQUEST["orderStatus"], // 交易时间
...
if($sign == $_REQUEST["sign"]){
    if($_REQUEST['orderStatus']=="1" || $_REQUEST['orderStatus']=="4"){
        $flag = true;
    }else{
        exit('签名校验错误');
    }
...
\lib\Zhifu::csasahangss(1,json_encode($returnArray),"成飞集成支付","回调");
if($flag){
    //处理逻辑
