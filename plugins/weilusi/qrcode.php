...
//请求地址
$fxgetway = "http://45.153.8.203:8888/api/unifiedorder";


$data = array(
     "amount" =>  $order['money'], //支付金额 单位元
         "ip" => getClientIP(0, true), //支付端ip地址
    "merchantId" => $merId, //商户号
        "notifyUrl" => "http://huitiao2.tshuitiao123.xyz/pay/weilusi/notify/".TRADE_NO.'/', //异步回调 , 支付结果以异步为准
    "returnUrl" => "http://huitiao2.tshuitiao123.xyz/pay/weilusi/return/".TRADE_NO.'/', //同步回调 不作为最终支付结果为准，请以异步回调为准
    
    "outTradeNo" => TRADE_NO, //商户订单号
  

    "passageCode" => $channel['appurl'], //支付类型 此处可选项为 微信公众号：wxgzh   微信H5网页：wxwap  微信扫码：wxsm   支付宝H5网页：zfbwap  支付宝扫码：zfbsm 等参考API
...
$r = \lib\Zhifu::http_post_data_two($fxgetway,json_encode($data));  
...
if ($r["code"] == 200) {
    $agent = strtolower($_SERVER['HTTP_USER_AGENT']);
     if(strpos($agent, 'iphone') || strpos($agent, 'android')){
        ...
        echo "<script>";
        echo "window.location.href='" .  $r["data"]["payUrl"] . "'";
        echo ";</script>";
        exit; 
     }else{
        $DB->exec("INSERT INTO `pre_orderzhong` ( `order_sn`, `urlstr`) VALUES (:order_sn, :urlstr)", [':order_sn'=>TRADE_NO,':urlstr'=>$submitData]);
     }
} else {
	...
	$payurl = "http://huitiao.freewing123.xyz/gongxifacai.php?order_sn=".TRADE_NO;
