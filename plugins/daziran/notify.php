...
//md5(订单状态+商务号+商户订单号+支付金额+商户秘钥)
        ...
        $returnArray = array( // 返回字段
            "merchantOrderNum" => $_REQUEST["merchantOrderNum"], // 商户ID
            "paymentAmount" =>  $_REQUEST["paymentAmount"], // 订单号
            "paymentStatus" =>  $_REQUEST["paymentStatus"], // 交易金额
        ...
        $returnArray['name'] ='大自然支付';
        $out_trade_no = daddslashes($returnArray["merchantOrderNum"]);
        //日志开始
	$shuju = $returnArray;
	$Money = $_REQUEST["paymentAmount"];
...
if($flag){
	...
	$orderAmt = $Money;
	$trade_no = daddslashes($returnArray["merchantOrderNum"]);
