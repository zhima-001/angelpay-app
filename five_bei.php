...
🅿" . $order_info2[0]['name'] . "\r\n";
    ...
    public function guangbo($chatid, $message, $from_id, $data)
    {
        ...
        $inline_keyboard_arr3 = array(
            array('text' => "昨日下发推送", "callback_data" => "paliangshang###5"),
...
1：已完成";
    ...
    public function xiafausdt($pid, $ubi, $usdt_fm, $message_id, $chatid, $data, $chatinfo, $type = "0", $tousu_U2)
    {
      ...
      if ($two_result['balance'] / 1000000 < $ubi) {
        ...
        if (!empty($three_result['txId'])) {
            ...
            $this->http_post_data('editMessageText', json_encode($parameter2));
            $this->xiaoxinoend("接口返回：--".$three_result['txId'],$chatid);
    ...
    {
        $username = "@" . $username;
//$this->xiaoxinoend($userid,$chatid);
  //      $this->xiaoxinoend(in_array($userid, $this->chaojiyonghu),$chatid);
        //var_dump(in_array($userid, $this->chaojiyonghu));
    ...
    public function yibudaowei($chatid)
    {
    ...
    public function quedingxiafa_xiafa($pid, $ubi, $usdt_fm, $message_id, $chatid,  $type = "0")
    {
        ...
        if ($xiafa_info3) {
            ...
            $this->http_post_data('sendMessage', json_encode($parameter));
            exit();*/
            return array(5,$pid,'前商户存在操作下发操作,但未收到USDT交易所返回的成功的信息，无法再次触发下发！请天使工作人员确定后，再手动下发剩余U币！');
        ...
        if (empty($ToAdress)) {
            //$this->xiaoxi("当前商户暂未设置下发USDT的地址,请核对后再下发！", $chat_id, '1', $data['callback_query']['id']);
            return array(6,$pid,'当前商户暂未设置下发USDT的地址,请核对后再下发！');
        ...
        if ($two_result['balance'] / 1000000 < $ubi) {
            ...
            $this->http_post_data('sendMessage', json_encode($parameter));
            exit();*/
            return array(7,$pid,"很抱歉，你的U币不足以下发,当前余额：" . $two_result['balance'] / 1000000);
        }
        $param_data = array(
            "ownerAddress" => $ownerAddress,
            "toAddress" => $ToAdress,
        ...
        if ($three_result['status'] == "500") {
            ...
            $this->http_post_data('sendMessage', json_encode($parameter));
            exit();*/
            return array(8,$pid,"转账下发失败，请联系天使客服,错误信息：" . $submitData3);
        ...
        if (!empty($three_result['txId'])) {
            ...
            if (count($zhifuxinxi) <= 0) {
                //$this->xiaoxi("当前商户暂未设置支付类型费率，请先设置！", $chatid);
                return array(8,$pid,"当前商户暂未设置支付类型费率，请先设置！");
            }
            ...
            if ($zuorixiafa) {
                ...
                return array(10,$pid,"当前商户昨日存在实时下发" . $zuorixiafa[0]['money'] . "U异常！建议手动结算昨日收益！");
            }
...
//                $inline_keyboard_arr[0] = array('text' => "下发异常!", "callback_data" => "yijingxiafa_" . $uid);
            ...
            $this->http_post_data('sendMessage', json_encode($parameter));*/
            return array(12,$uid,'最后环节下发失败，请联系天使客服');
        }
