...
$userrow = $DB->getRow("SELECT uid,gid,mode,userstype FROM pre_user WHERE uid='{$order['uid']}' LIMIT 1");

// 获取订单支付方式ID、支付插件、支付通道、支付费率
$submitData = \lib\Channel::submit2($typeid, $userrow['gid'],(int)$order['money'],$order['terminals'],$_GET['stype'],(int)$userrow['userstype']);
