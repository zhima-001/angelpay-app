...
$order['money'] = $order['realmoney'];
	...
	$server_name = $url1 . $_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']."<br>";
	if (strpos($server_name, 'djs') == false) {
		xieCuowu1("订单号：".TRADE_NO.'&nbsp;&nbsp;访问页面：'.$server_name,TRADE_NO);
	}
include $loadfile;