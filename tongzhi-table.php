...
$sql=" 1=1 and A.trade_no in (select trade_no from pre_hongbao where miyao='{$miyao}') and A.status<>1 and A.addtime>'".date("Y-m-d H:i:s",(time()-300))."'";
$links='&miyao='.$_GET['miyao'];
...
?>
	  <div class="table-responsive">
        <table class="table table-striped table-bordered table-vcenter">
          <thead><tr><th>系统订单号/商户订单号</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
//echo "SELECT A.*,B.plugin FROM pre_order A LEFT JOIN pre_channel B ON A.channel=B.id WHERE{$sql} order by trade_no desc limit $offset,$pagesize";
while($res = $rs->fetch())
{
echo '<tr><td>'.$res['trade_no'].'<br/>'.$res['out_trade_no'].'<br/>'.$res['addtime'].'<br/>'.$res['endtime'].'</td><td>￥ <b>'.$res['money'].'</b><br>'.display_status($res['status'], $res['notify']).'</td><td><a href="javascript:callnotify(\''.$res['trade_no'].'\')" class="btn btn-success btn-xs">回调</a><br><br>'.$res['sjm'].'</td></tr>';
}
?>
          </tbody>
        </table>
