...
if($_GET['do']=='settle'){
	if($conf['settle_open']==1 || $conf['settle_open']==3){
		...
		//echo "SELECT * from pre_user where money>={$conf['settle_money']} and account is not null and username is not null and settle=1 and status=1";
		$i=0;
		$allmoney=0;
		while($row = $rs->fetch())
		{
...
/*	if(strtotime($order_time)>=strtotime(date("Y-m-d").' 00:00:00'))exit('订单统计与清理任务今日已完成');*/
	...
	$thtime=date("Y-m-d H:i:s",time()-3600*24*30);

	$CACHE->clean();
	$DB->exec("delete from pre_order where  addtime<'{$thtime}'"); 
